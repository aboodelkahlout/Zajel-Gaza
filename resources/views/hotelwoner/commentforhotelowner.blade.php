...
<body>
    ...
    <div class="review-item" style="border: 1px solid #ccc; padding: 15px; margin-bottom: 15px;">
        ...
        <div class="review-actions" style="margin-top: 10px;">
            <form action="{{ route('hotel.destroycomment', $comment->id) }}" method="POST" style="display: inline;">
                @csrf
                @method('DELETE')
        ...
        <div class="review-reply" style="margin-top: 10px; background: #f9f9f9; padding: 10px; border-right: 3px solid #2980b9;">
                ...
                <a class="btn btn-primary" href="{{ route('hotelowner.show.edit.reply', [$comment->id, $reply->id]) }}" class="action-btn">
                <i class="fas fa-reply"></i> تعديل الرد
            </a>
            <form class="d-inline" action="{{ route('hotelowner.destroy.reply', $reply->id) }}" method="post">
                @csrf
                @method('DELETE')
            <button class="btn btn-danger" class="action-btn">
                <i class="fas fa-trash ms-2"></i>حذف الرد</button>
            </form>
            </div>
        @endforeach
        @endif
            <a href="{{ route('hotelwoner.commentsreplyForm', $comment->id) }}" class="action-btn reply-btn">
